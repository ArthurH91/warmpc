...
class TrajGeneration():
    ...
    def generate_traj(self, X0, targ):
        ...
        try:
            ...
            inputs, outputs = self.from_xs_to_input_output(target, xs)
    ...
    def from_xs_to_input_output(self, targ: np.ndarray, xs: list) -> Tuple[torch.Tensor, torch.Tensor]:
        
        X0 = xs[0]
        XT = xs[-1]
        inputs_tensor = torch.tensor(np.concatenate((X0[:7], XT[:7])), dtype=torch.float32)
...
if __name__ == "__main__":
    ...
    TG.save_results_as_tensors(results, 'data_100.pt')
